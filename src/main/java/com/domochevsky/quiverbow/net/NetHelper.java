...
public class NetHelper
{
    ...
    public static void sendParticleMessageToAllPlayers(World world, int entityID, EnumParticleTypes particle,
	    byte strength)
    {
	if (world.isRemote)
	{
    ...
    public static void sendParticleMessage(Entity user, int entityID, EnumParticleTypes particle, byte strength)
    {
    ...
    public static void sendTurretStateMessageToPlayersInRange(World world, Entity_AA turret, boolean hasArmor,
	    boolean hasWeaponUpgrade, boolean hasRidingUpgrade, boolean hasPlatingUpgrade, boolean hasComUpgrade)
    {
	...
	while (counter < list.size())
	{
	    ...
	    if (potentialPlayer instanceof EntityPlayer)
	    {
		sendTurretStateMessageToPlayer((EntityPlayer) potentialPlayer, turret.getEntityId(), hasArmor,
			hasWeaponUpgrade, hasRidingUpgrade, hasPlatingUpgrade, hasComUpgrade);
	    }
    ...
    public static void sendTurretInventoryMessageToPlayersInRange(World world, Entity_AA turret, int itemID,
	    int itemSlot, int metadata)
    {
	// Step 1, who's in range?
	if (world.isRemote)
	{
	...
	while (counter < list.size())
	{
	    ...
	    if (potentialPlayer instanceof EntityPlayer)
	    {
		sendTurretInventoryMessageToPlayer((EntityPlayer) potentialPlayer, turret.getEntityId(), itemID,
			itemSlot, metadata);
	    }
	    // else, not a player. Don't care
    ...
    private static void sendTurretStateMessageToPlayer(EntityPlayer player, int turretEntityID, boolean hasArmor,
	    boolean hasWeaponUpgrade, boolean hasRidingUpgrade, boolean hasPlatingUpgrade, boolean hasComUpgrade)
    {
	IMessage msg = new TurretStateMessage(turretEntityID, hasArmor, hasWeaponUpgrade, hasRidingUpgrade,
		hasPlatingUpgrade, hasComUpgrade);
	PacketHandler.net.sendTo(msg, (EntityPlayerMP) player);
    }

    private static void sendTurretInventoryMessageToPlayer(EntityPlayer player, int turretEntityID, int itemID,
	    int itemSlot, int metadata)
    {
