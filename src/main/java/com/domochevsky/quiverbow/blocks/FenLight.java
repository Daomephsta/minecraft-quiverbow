...
public class FenLight extends Block
{
	...
	if (!world.isRemote)
	{
	    ...
	    if (meta == 0) { if ( !world.isSideSolid(x, y + 1, z, ForgeDirection.DOWN) ) { world.setBlockToAir(x, y, z); } } // Bottom, checking Top
	    else if (meta == 1) { if ( !world.isSideSolid(x, y - 1, z, ForgeDirection.UP) ) { world.setBlockToAir(x, y, z); } } // Top, checking Bottom
	    else if (meta == 2) { if ( !world.isSideSolid(x, y, z + 1, ForgeDirection.NORTH) ) { world.setBlockToAir(x, y, z); } }	// East
	    else if (meta == 3) { if ( !world.isSideSolid(x, y, z - 1, ForgeDirection.SOUTH) ) { world.setBlockToAir(x, y, z); } }	// West
	    else if (meta == 4) { if ( !world.isSideSolid(x + 1, y, z, ForgeDirection.WEST) ) { world.setBlockToAir(x, y, z); } }	// North
	    else if (meta == 5) { if ( !world.isSideSolid(x - 1, y, z, ForgeDirection.EAST) ) { world.setBlockToAir(x, y, z); } }	// South
	}
    }
