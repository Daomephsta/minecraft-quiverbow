...
public class RecipeWeapon extends ShapedRecipes implements IRecipe
{
	private int upgradeType;

	public RecipeWeapon(ItemStack[] components, ItemStack result, int upgradeType)
	{
	...
	public boolean matches(InventoryCrafting matrix, World world)
	{
		...
		{
			return false;
		} // Only care about full size crafting recipes

		// if (!world.isRemote) { System.out.println("[RECIPE] Checking for
		// recipe matches."); }
		...
		while (currentSlot < 9) // Going through all 9 slots
		{
			...
			if (!currentStack.isEmpty() && !this.recipeItems[currentSlot].isEmpty())
			{
				...
				else if (currentStack.getItemDamage() != this.recipeItems[currentSlot].getItemDamage())
				{
					return false;
				} // Damage doesn't match up
				else if (currentStack.isStackable()
						&& currentStack.getCount() < this.recipeItems[currentSlot].getCount())
				{
					return false;
				} // Not the right amount
				else
				{
					if (currentStack.getItem() instanceof WeaponBase) // This
					// is the
					// weapon
					// in
					// question
					{
						...
						if (this.upgradeType == 1) // Emerald Muzzle, for the
						// ERA
						{
							if (currentStack.hasTagCompound()
									&& currentStack.getTagCompound().getBoolean("hasEmeraldMuzzle"))
							{
								return false;
							} // Already upgraded
						}
	...
	@Override
	public ItemStack getCraftingResult(InventoryCrafting crafting) // Making it
	// happen
	{
		...
		{
			stack.getTagCompound().setBoolean("hasEmeraldMuzzle", true); // you
			// now
			// have
			// an
			// upgrade
