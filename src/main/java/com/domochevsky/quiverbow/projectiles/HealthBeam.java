...
public class HealthBeam extends _ProjectileBase implements IEntityAdditionalSpawnData
{
    ...
    public void doSetup(Entity entity, Entity target, float setYaw, float setPitch) // Default
										    // setup
										    // for
										    // all
										    // shot
	...
	{
	    if (movPos.entityHit instanceof EntityLivingBase) // We hit a LIVING
							      // living thing!
	    {
		EntityLivingBase living = (EntityLivingBase) movPos.entityHit;

		if (living.isEntityUndead()) // Not affected by regen potions,
					     // so whithering them instead
		{
		    Helper.applyPotionEffect(living, new PotionEffect(MobEffects.WITHER, 20, 3));
		}
		else // Applying regen
		{
		    // Healing
		    ...
		    if (health >= healthMax && living.getAbsorptionAmount() < healthMax) // Can
											 // be
											 // buffed
											 // up
											 // to
		    ...
		    // else, either not at full health or already full up on
		    // absorption

		    Helper.applyPotionEffect(living, new PotionEffect(MobEffects.REGENERATION, 20, 3));
		}
	    }
	}
